...
NS_LOG_COMPONENT_DEFINE("ndn.StackHelperManip");

namespace ns3 {
namespace ndn {

StackHelperManip::StackHelperManip()
    ...
    m_netDeviceCallbacks.push_back(std::make_pair(
        ...
        MakeCallback(&StackHelperManip::PointToPointNetDeviceCallback, this)));
...
void StackHelperManip::setCsSize(size_t maxSize) { m_maxCsSize = maxSize; }

void StackHelperManip::setPolicy(const std::string& policy) {
...
void StackHelperManip::Install(const NodeContainer& c) const {
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i) {
        Install(*i);
    }
}

void StackHelperManip::InstallAll() const {
    Install(NodeContainer::GetGlobal());
}

void StackHelperManip::Install(Ptr<Node> node) const {
    ...
    Simulator::ScheduleWithContext(node->GetId(), Seconds(0),
                                   &StackHelperManip::doInstall, this, node);
    ProcessWarmupEvents();
}

void StackHelperManip::doInstall(Ptr<Node> node) const {
...
void StackHelperManip::AddFaceCreateCallback(
    TypeId netDeviceType, StackHelperManip::FaceCreateCallback callback) {
    m_netDeviceCallbacks.push_back(std::make_pair(netDeviceType, callback));
}

void StackHelperManip::UpdateFaceCreateCallback(TypeId netDeviceType,
...
void StackHelperManip::RemoveFaceCreateCallback(TypeId netDeviceType,
                                                FaceCreateCallback callback) {
...
shared_ptr<Face> StackHelperManip::DefaultNetDeviceCallback(
    ...
    opts.defaultCongestionThreshold = MARKTHOLD;

    auto linkService = make_unique<::nfd::face::GenericLinkService>(opts);

    auto transport = make_unique<NetDeviceTransportManip>(
...
shared_ptr<Face> StackHelperManip::PointToPointNetDeviceCallback(
    ...
    ndn->addFace(face);
    NS_LOG_LOGIC("Node " << node->GetId() << ": added Face" << face->getId()
                         << " as face #" << face->getLocalUri());

    return face;
}

void StackHelperManip::Install(const std::string& nodeName) const {
    Ptr<Node> node = Names::Find<Node>(nodeName);
    Install(node);
}

void StackHelperManip::Update(Ptr<Node> node) {
    if (node->GetObject<L3Protocol>() == 0) {
        Install(node);
        return;
    }
...
void StackHelperManip::Update(const NodeContainer& c) {
    for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i) {
        Update(*i);
    }
}

void StackHelperManip::Update(const std::string& nodeName) {
    Ptr<Node> node = Names::Find<Node>(nodeName);
    Update(node);
}

void StackHelperManip::UpdateAll() { Update(NodeContainer::GetGlobal()); }

shared_ptr<Face> StackHelperManip::createAndRegisterFace(
    Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const {
...
void StackHelperManip::disableStrategyChoiceManager() {
    m_isStrategyChoiceManagerDisabled = true;
}

void StackHelperManip::disableForwarderStatusManager() {
    m_isForwarderStatusManagerDisabled = true;
}

void StackHelperManip::SetLinkDelayAsFaceMetric() {
    for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i) {
        ...
        for (auto& face : ndn->getFaceTable()) {
            auto transport =
                dynamic_cast<NetDeviceTransportManip*>(face.getTransport());
...
void StackHelperManip::ProcessWarmupEvents() {
