...
const time::milliseconds
    BestRouteStrategy2WithOutFaceWnd::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds BestRouteStrategy2WithOutFaceWnd::RETX_SUPPRESSION_MAX(
    250);

BestRouteStrategy2WithOutFaceWnd::BestRouteStrategy2WithOutFaceWnd(
    Forwarder &forwarder, const Name &name)
    : Strategy(forwarder),
      ...
      dTimer(ns3::Seconds(0.1), 1024, this) {
    ParsedInstanceName parsed = parseInstanceName(name);
    if (!parsed.parameters.empty()) {
        NDN_THROW(std::invalid_argument(
            "BestRouteStrategy2WithOutFaceWnd does not accept parameters"));
    }
    if (parsed.version &&
        *parsed.version != getStrategyName()[-1].toVersion()) {
        NDN_THROW(std::invalid_argument(
            "BestRouteStrategy2WithOutFaceWnd does not support version " +
    ...
    forwarder.beforeExpirePendingInterest.connect(
        [this](const nfd::pit::Entry &entry) {
            for (const pit::OutRecord &i : entry.getOutRecords()) {
                ...
                // std::cout << "OnExpire Decrease, face id(" <<
                // i.getFace().getId() << "), Pending: " <<
                // facePending[i.getFace().getId() % 16 - 2] << std::endl;
            }
        });
}
...
const Name &BestRouteStrategy2WithOutFaceWnd::getStrategyName() {
    static Name strategyName(
        "/localhost/nfd/strategy/best-route2-conges-multiport/%FD%01");
    return strategyName;
}

void BestRouteStrategy2WithOutFaceWnd::afterReceiveInterest(
    const Interest &interest, const FaceEndpoint &ingress,
...
bool BestRouteStrategy2WithOutFaceWnd::canSendInt(
    ...
    faceRttRec[index].insert(
        std::make_pair(interest.getName(), ns3::Simulator::Now()));

    // std::cout << "OnInterest Increase, " << "face: " << egress << "\tPending:
    // " << facePending[index] << std::endl;
    return true;
}

void BestRouteStrategy2WithOutFaceWnd::afterReceiveNack(
    const lp::Nack &nack, const FaceEndpoint &ingress,
    const shared_ptr<pit::Entry> &pitEntry) {
    int index = ingress.face.getId() % 16 - 2;
    if (nack.getReason() != lp::NackReason::FREE &&
        nack.getReason() != lp::NackReason::BUSY) {
        facePending[index]--;
        faceRttRec[index].erase(nack.getInterest().getName());
        // std::cout << "OnNack Decrease, " << "face: " << ingress <<
        // "\tPending: " << facePending[index] << std::endl;
    }
    this->processNack(nack, ingress.face, pitEntry);
}

void BestRouteStrategy2WithOutFaceWnd::beforeSatisfyInterest(
    const Data &data, const FaceEndpoint &ingress,
    const shared_ptr<pit::Entry> &pitEntry) {
    if (ingress.face.getId() != face::FACEID_CONTENT_STORE) {
        ...
        ns3::Time difftime =
            ns3::Simulator::Now() - faceRttRec[index][data.getName()];
        if (faceFirst[index]) {
            faceRttEstm[index] = difftime.GetSeconds();
            faceFirst[index] = false;
        }
        if (difftime.GetNanoSeconds() != 0) {
            faceRttEstm[index] =
                faceRttEstm[index] * 0.75 + 0.25 * difftime.GetSeconds();
...
void BestRouteStrategy2WithOutFaceWnd::afterContentStoreHit(
    const Data &data, const FaceEndpoint &ingress,
    const shared_ptr<pit::Entry> &pitEntry) {
    // int index = ingress.face.getId() % 16 - 2;
    // facePending[index]++;
...
ddpgTimerTrans::ddpgTimerTrans(ns3::Time interval, uint16_t id,
    ...
    : ns3::Ns3AIRL<edgeDDPGEnv, edgeDDPGAct>(id), intervalt(interval) {
    Ping(intervalt);
    SetFunction(BestRouteStrategy2WithOutFaceWnd::callback);
    SetArguments<BestRouteStrategy2WithOutFaceWnd *>(std::move(ptr));
    SetCond(2, 0);
}

void BestRouteStrategy2WithOutFaceWnd::callback(
    BestRouteStrategy2WithOutFaceWnd *ptr) {
    std::cout << std::setw(64) << std::setfill('=') << '=' << std::setfill(' ')
              << std::endl;
    std::cout << "time:" << ns3::Simulator::Now().GetSeconds() << std::endl;
    std::cout << std::setiosflags(std::ios::left) << std::setw(12) << "face"
              << std::setw(12) << "Cwndmax" << std::setw(12) << "Pending"
              << std::setw(12) << "DataNum" << std::setw(12) << "RttEstm"
              << std::endl;
    for (int i = 0; i < 3; i++) {
        std::cout << std::setiosflags(std::ios::left) << std::setw(12) << i
                  << std::setw(12) << ptr->faceCwndMax[i] << std::setw(12)
                  << ptr->facePending[i] << std::setw(12) << ptr->faceDataNum[i]
                  << std::setw(12) << ptr->faceRttEstm[i] << std::endl;
    }
    std::cout << std::setw(64) << std::setfill('-') << '-' << std::setfill(' ')
              << std::endl;
    ...
    for (int i = 0; i < 3; i++) {
        std::cout << std::setiosflags(std::ios::left) << std::setw(12) << i
                  << std::setw(12) << ptr->faceCwndMax[i] << std::endl;
    }
    std::cout << std::setw(64) << std::setfill('=') << '=' << std::setfill(' ')
              << std::endl;
