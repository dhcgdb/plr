...
#ifndef NDNSIM_HELPER_NDN_STACK_HELPER_M_HPP
#define NDNSIM_HELPER_NDN_STACK_HELPER_M_HPP
 ...
 */
class StackHelperManip : boost::noncopyable {
  public:
    /**
     * \brief Create a new NdnStackHelper with a default NDN_FLOODING forwarding
     * stategy
     */
    StackHelperManip();
     ...
     */
    virtual ~StackHelperManip();

    /**
     * @brief Set parameters of NdnL3Protocol
     */
    void SetStackAttributes(
    ...
    typedef Callback<shared_ptr<Face>, Ptr<Node>, Ptr<L3Protocol>,
                     Ptr<NetDevice>>
        FaceCreateCallback;

    /**
     * @brief Add callback to create and configure instance of the face, based
     *on supplied Ptr<Node> and Ptr<NetDevice>
     *
     * It is possible to set up several callbacks for different NetDevice types.
     *
     * If none of the callbacks fit the TypeId of NetDevice, a default callback
     *is used (DefaultNetDeviceCallback)
     */
    void AddFaceCreateCallback(TypeId netDeviceType,
                               FaceCreateCallback callback);

    /**
     * @brief Update callback to create and configure instance of the face,
     *based on supplied Ptr<Node> and Ptr<NetDevice>
     *
     * It is possible to set up several callbacks for different NetDevice types.
     *
     * Using this method, it is possible to override Face creation for
     *PointToPointNetDevices
     */
    void UpdateFaceCreateCallback(TypeId netDeviceType,
                                  FaceCreateCallback callback);

    /**
     * @brief Remove callback to create and configure instance of the face,
     * based on supplied Ptr<Node> and Ptr<NetDevice>
     */
    void RemoveFaceCreateCallback(TypeId netDeviceType,
                                  FaceCreateCallback callback);
     ...
     * node that already has an ndn object aggregated to it.
     *
     * \param c NodeContainer that holds the set of nodes on which to install
     * the new stacks.
     *
     * \returns list of installed faces in the form of a smart pointer
     * to FaceContainer object
    ...
    /**
     * @brief Set face metric of all faces connected through PointToPoint
     * channel to channel latency
     */
  ...
  private:
    shared_ptr<Face> DefaultNetDeviceCallback(Ptr<Node> node,
                                              Ptr<L3Protocol> ndn,
                                              Ptr<NetDevice> netDevice) const;

    shared_ptr<Face> PointToPointNetDeviceCallback(
        Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> netDevice) const;
    shared_ptr<Face> createAndRegisterFace(Ptr<Node> node, Ptr<L3Protocol> ndn,
  ...
  private:
    ...
    typedef std::function<std::unique_ptr<nfd::cs::Policy>()>
    ...
    typedef std::list<std::pair<TypeId, FaceCreateCallback>>
        NetDeviceCallbackList;
