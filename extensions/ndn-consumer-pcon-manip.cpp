...
#include "ndn-consumer-pcon-manip.hpp"

NS_LOG_COMPONENT_DEFINE("ndn.ConsumerPconManip");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(ConsumerPconManip);

constexpr double ConsumerPconManip::CUBIC_C;
constexpr uint32_t ConsumerPconManip::BIC_MAX_INCREMENT;
constexpr uint32_t ConsumerPconManip::BIC_LOW_WINDOW;

TypeId ConsumerPconManip::GetTypeId() {
    static TypeId tid =
        TypeId("ns3::ndn::ConsumerPconManip")
            .SetGroupName("Ndn")
            .SetParent<ConsumerWindow>()
            .AddConstructor<ConsumerPconManip>()
            ...
            .AddAttribute(
                ...
                MakeEnumAccessor(&ConsumerPconManip::m_ccAlgorithm),
                MakeEnumChecker(CcAlgorithm::AIMD, "AIMD", CcAlgorithm::BIC,
                                "BIC", CcAlgorithm::CUBIC, "CUBIC"))

            .AddAttribute("Beta", "TCP Multiplicative Decrease factor",
                          DoubleValue(0.5),
                          MakeDoubleAccessor(&ConsumerPconManip::m_beta),
                          MakeDoubleChecker<double>())

            .AddAttribute("CubicBeta",
                          "TCP CUBIC Multiplicative Decrease factor",
                          DoubleValue(0.8),
                          MakeDoubleAccessor(&ConsumerPconManip::m_cubicBeta),
                          MakeDoubleChecker<double>())
            ...
            .AddAttribute(
                ...
                "decreases",
                DoubleValue(
                    0.5),  // This default value was chosen after manual testing
                MakeDoubleAccessor(&ConsumerPconManip::m_addRttSuppress),
                MakeDoubleChecker<double>())
            ...
            .AddAttribute("ReactToCongestionMarks",
                          ...
                          BooleanValue(true),
                          MakeBooleanAccessor(
                              &ConsumerPconManip::m_reactToCongestionMarks),
                          MakeBooleanChecker())

            .AddAttribute("UseCwa",
                          "If true, use Conservative Window Adaptation",
                          BooleanValue(true),
                          MakeBooleanAccessor(&ConsumerPconManip::m_useCwa),
                          MakeBooleanChecker())

            .AddAttribute(
                "UseCubicFastConvergence",
                "If true, use TCP CUBIC Fast Convergence", BooleanValue(false),
                MakeBooleanAccessor(&ConsumerPconManip::m_useCubicFastConv),
                MakeBooleanChecker())

            .AddAttribute(
                "ReactToNack", "", BooleanValue(false),
                MakeBooleanAccessor(&ConsumerPconManip::m_reactToNack),
                MakeBooleanChecker());

    return tid;
}

ConsumerPconManip::ConsumerPconManip()
    : m_ssthresh(100),  // std::numeric_limits<double>::max()),
      m_highData(0),
      m_recPoint(0.0),
      m_cubicWmax(0),
      m_cubicLastWmax(0),
...
void ConsumerPconManip::OnData(shared_ptr<const Data> data) {
    ...
    if (data->getCongestionMark() > 0) {
        if (m_reactToCongestionMarks) {
            ...
            NS_LOG_DEBUG("Ignored received congestion mark: "
...
void ConsumerPconManip::OnTimeout(uint32_t sequenceNum) {
    WindowDecrease();
...
void ConsumerPconManip::OnNack(shared_ptr<const lp::Nack> nack) {
    uint64_t nackSeq = nack->getInterest().getName().get(-1).toSequenceNumber();
    if (m_reactToNack) {
        NS_LOG_DEBUG("OnNack" << this << " seq:" << nackSeq);
        WindowDecrease();
        //OnTimeout(nackSeq);
    } else {
        NS_LOG_DEBUG("Ignore Nack seq:" << nackSeq);
    }
    NS_LOG_DEBUG("Nack reasons:" << nack->getReason());
    Consumer::OnNack(nack);
}

void ConsumerPconManip::WindowIncrease() {
...
void ConsumerPconManip::WindowDecrease() {
    if (!m_useCwa || m_highData > m_recPoint) {
        ...
        NS_LOG_DEBUG("Window decrease suppressed, HighData: "
                     << m_highData << ", RecPoint: " << m_recPoint);
    }
}

void ConsumerPconManip::BicIncrease() {
...
void ConsumerPconManip::BicDecrease() {
    // BIC Decrease
    if (m_window >= BIC_LOW_WINDOW) {
...
void ConsumerPconManip::CubicIncrease() {
    ...
    //  const double w_est = m_cubic_wmax*m_beta + (3*(1-m_beta)/(1+m_beta)) *
    //  (t/rtt);
    ...
    if (m_window < m_ssthresh) {
        ...
        // Note: This change is not part of the RFC, but I added it to improve
        // performance.
...
void ConsumerPconManip::CubicDecrease() {
