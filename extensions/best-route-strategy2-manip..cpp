...
#include "best-route-strategy2-manip.hpp"
...
NFD_LOG_INIT(BestRouteStrategy2Manip);
NFD_REGISTER_STRATEGY(BestRouteStrategy2Manip);

const time::milliseconds BestRouteStrategy2Manip::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds BestRouteStrategy2Manip::RETX_SUPPRESSION_MAX(250);

BestRouteStrategy2Manip::BestRouteStrategy2Manip(Forwarder& forwarder,
                                                 const Name& name)
    : Strategy(forwarder),
    ...
    if (!parsed.parameters.empty()) {
        NDN_THROW(std::invalid_argument(
            "BestRouteStrategy2Manip does not accept parameters"));
    }
    if (parsed.version &&
        *parsed.version != getStrategyName()[-1].toVersion()) {
        NDN_THROW(std::invalid_argument(
            "BestRouteStrategy2Manip does not support version " +
            to_string(*parsed.version)));
    }
    this->setInstanceName(makeInstanceName(name, getStrategyName()));
}

const Name& BestRouteStrategy2Manip::getStrategyName() {
    static Name strategyName("/localhost/nfd/strategy/best-route-manip/%FD%05");
    return strategyName;
}

void BestRouteStrategy2Manip::afterReceiveInterest(
...
void BestRouteStrategy2Manip::afterReceiveNack(
    const FaceEndpoint& ingress, const lp::Nack& nack,
    const shared_ptr<pit::Entry>& pitEntry) {
    lp::NackHeader outNack;
    outNack.setReason(nack.getReason());
    sendNacks(pitEntry, outNack);
}
